...
translateBind (Rec list) =
  let (var, exprBind) = list !! 0
   in translateCoreBndr var exprBind
...
splitCaseClauses varAlt ciaohead@(CiaoTerm _ args) (alt : restOfAlts) = do
  ...
  where
    funbody = \(_, _, expr) -> translateFunBody expr
    funhead :: CoreAlt -> Env CiaoHead
    funhead = \(altcon, conargs, _) ->
      let posArg = findInArgList (CiaoId $ (hsIDtoCiaoVarID . show) $ varAlt) args
       in case posArg of
                    ...
                    where
                      ...
                      ciaoArgList =
                        sequence $
                          map
                            ( \x ->
                                (put $ env {usedIdsInBody = x : (usedIdsInBody env)} :: Env ())
                                  >> (return $ CiaoArgTerm $ CiaoTerm (CiaoId $ ((hsIDtoCiaoVarID . showHsID env) x)) [])
                            )
                            conargs
                  _ -> return ciaohead
...
dollarFilter str =
  let droppedDollar = dropWhile (/= '$') str
   in case droppedDollar of
        [] -> str
        _ -> droppedDollar
...
translateFunBody (Case app _ _ altlist) =
  let var = getCoreVarFromAppTree app
   in case var of
...
collectLetBinds (Rec list) =
  let (_, exprBind) = list !! 0
   in collectFromBndr exprBind

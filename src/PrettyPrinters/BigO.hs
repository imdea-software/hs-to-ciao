...
printBigOExpr singleOAnalysis =
  ...
  where
    ...
    (lambdaName, lambdaSetOfArgs) = case singleOAnalysis =~ "pred (.*)\\((.*)\\)" of
      [] ->
        error
          "Couldn't find the arguments of the BigO function in the CiaoPP \
          \ analysis output file. This isn't the user's fault."

...
tryToGetNeededPredicates ciaoPreludeContents ciaoFunctor =
    ...
    where neededPredicates = fromFunctorGetNeededPredicates ciaoPreludeContents ciaoFunctor
                  
errSubfunctorNotFound :: CiaoFunctor -> Either String String -> IO (Maybe String)
errSubfunctorNotFound functor subfunctor =
    case subfunctor of
      (Left missingFunctor) -> (putStrLn $ "WARNING: The predicate \"" ++
                               missingFunctor ++ "\", which is required by " ++
...
errSomePredNotFound subfunctor =
    case subfunctor of
      (Left missingFunctor) -> (putStrLn $ "WARNING: The predicate \"" ++
                               missingFunctor ++ "\", which could be required by" ++

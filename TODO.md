...
- TODO
  - [ ] Make the mergeSort and quickSort, ghc's smooth merge sort examples work
    - [ ] Implement Let within the Ciao syntax and the translation
  - [x] Make the `showQualified` function modular
    - [x] Add a Reader monad to the whole translation
  - [ ] Add to CiaoFunctor in the syntax the arity of the functor (and use record syntax for fields)
...
- TODO
  - [ ] CiaoPP connection 
    ...
    - [x] write a summary of the status of the CiaoPP issue (and goals) to the Ciao team and include Niki in the email
  - [x] try to translate insertionSort (maybe mergeSort and quickSort, ghc's smooth merge sort later on)
    - [x] ignore the patError (dead code) that gets generated by GHC
    - [x] translate (<), (+) etc. 
      - [x] print (+), (<), etc with full names (i.e., include module names) since (+) is already taken by CiaoPP, but `GHC.Num.+` is not
        - [x] integrate `showQualified` function properly in the translation
      - [x] Print `GHC.Num.+` etc using ONLY CiaoPP friendly symbols, i.e., no `.` no `+`.
      - [x] Define a library that translates `GHCNumPlus(D, X, Y)` into `X+Y`
